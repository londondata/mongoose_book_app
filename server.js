...
app.get('/api/books/:id', function (req, res) {
  ...
  db.Book.findById({_id: bookId }, (err, foundBook) => {
    if(err) {return console.log(err) }
      res.json(foundBook);
       // we found the right book, we can stop searching
    });
});

//from above code b4 database 
...
// get book id from url params (`req.params`)
  ...
  let updateBody = req.body;
    ...
    db.Todo.findOneAndUpdate ({ _id:bookId }, updateBody, {new:true}, (err, updatedBook) => {
  if (err) {return console.log(err)}
